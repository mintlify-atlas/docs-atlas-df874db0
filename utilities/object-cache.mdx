...
```typescript
import { MemoryObjectCache } from '@moqtail/moqtail-ts'
import { Location } from '@moqtail/moqtail-ts'
...
```typescript
import { RingBufferObjectCache } from '@moqtail/moqtail-ts'
...
Object caches are commonly used with `StaticContentSource` for on-demand content delivery:

### Static Content

```typescript
import { StaticContentSource, MemoryObjectCache } from '@moqtail/moqtail-ts'
import { FullTrackName, ObjectForwardingPreference } from '@moqtail/moqtail-ts'
...
const contentSource = new StaticContentSource(fileCache)
...
```typescript
import { HybridContentSource, RingBufferObjectCache } from '@moqtail/moqtail-ts'
...
const hybridSource = new HybridContentSource({
  liveStream: videoObjectStream,
  cache: recentCache,
})
...
async function createVODTrack(videoId: string, segments: Uint8Array[]) {
  ...
  const contentSource = new StaticContentSource(vodCache)
...
<CardGroup cols={2}>
  <Card title="Content Sources" href="/concepts/content-sources">
    Learn about LiveContentSource, StaticContentSource, and HybridContentSource
  </Card>
